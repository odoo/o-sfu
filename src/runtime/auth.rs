    ...
    T: Serialize,
{
    ...
    // `claims` can carry a room key or key seed (`HttpRoomClaims`), so each
    // plaintext copy made while assembling the token is zeroized as soon as
    // the next copy derived from it exists, rather than left for a plain
    // drop to merely deallocate without clearing.
    let mut claims_json =
        serde_json::to_vec(claims).map_err(|_error| AuthenticationError::InvalidJsonPayload)?;
    let header_b64 = URL_SAFE_NO_PAD.encode(header_json);
    let mut claims_b64 = URL_SAFE_NO_PAD.encode(&claims_json);
    claims_json.zeroize();
    let mut signed_data = format!("{header_b64}.{claims_b64}");
    claims_b64.zeroize();
    let signature = sign_hs256(signed_data.as_bytes(), &key)?;
    let signature_b64 = URL_SAFE_NO_PAD.encode(signature);
    let token = format!("{signed_data}.{signature_b64}");
    signed_data.zeroize();
    ...
    T: DeserializeOwned,
{
    ...
    let mut claims_bytes = decode_jwt_segment(claims_b64)?;
