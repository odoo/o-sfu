...
fn jwt_claims_round_trip_room() -> serde_json::Result<()> {
    let room_claims = HttpRoomClaims {
        ...
        key: Some(SecretString::from("Y2hhbm5lbC1rZXk=")),
        key_seed: None,
    };
    let room_claims_test = TestHttpRoomClaims::from(&room_claims);
    ...
    let room_claims_from_json = serde_json::from_value::<HttpRoomClaims>(expected_room_claims)?;
    assert_eq!(
        TestHttpRoomClaims::from(&room_claims_from_json),
        room_claims_test
    );
...
fn sign_and_verify_round_trip() {
    ...
    let verified = verify::<HttpRoomClaims>(&token, &SecretString::from(TEST_AUTH_KEY));
...
fn sign_and_verify_round_trip_with_uuid_like_channel_key() {
    ...
    let token = sign(
        &claims,
        &SecretString::from("123e4567-e89b-12d3-a456-426614174000"),
    );
    assert!(token.is_ok());
    let Some(token) = token.ok() else {
        return;
...
fn decode_key_matches_legacy_uuid_like_channel_key_bytes() {
    let decoded = decode_key(&SecretString::from("123e4567-e89b-12d3-a456-426614174000"));
    assert_eq!(
        decoded
            .ok()
...
fn verify_rejects_expired_token() {
    ...
    let error = verify::<HttpRoomClaims>(&token, &SecretString::from(TEST_AUTH_KEY)).err();
...
fn sign_emits_jose_base64url_segments() {
    ...
    let segments = token.expose_secret().split('.').collect::<Vec<_>>();
...
fn verify_does_not_parse_claims_before_signature_verification() {
    ...
    let invalid_claims_json =
        replace_token_segment(token.expose_secret(), 1, &URL_SAFE_NO_PAD.encode(b"{"));
...
fn sign_token_for_test<T: Serialize>(
    ...
    let key = decode_key(&SecretString::from(key_b64)).ok()?;
