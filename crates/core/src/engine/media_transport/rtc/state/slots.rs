...
pub(in super::super) struct SessionSlot;
...
pub(in super::super) struct SlotHandle<Tag> {
    index: usize,
    generation: u64,
...
/// ```
pub(in super::super) struct SlotStore<T, Tag> {
...
impl<T, Tag> SlotStore<T, Tag> {
    ...
    pub(in super::super) fn insert(&mut self, value: T) -> SlotHandle<Tag> {
    ...
    pub(in super::super) fn get(&self, handle: SlotHandle<Tag>) -> Option<&T> {
    ...
    pub(in super::super) fn get_mut(&mut self, handle: SlotHandle<Tag>) -> Option<&mut T> {
    ...
    pub(in super::super) fn values_mut(&mut self) -> impl Iterator<Item = &mut T> {
        self.entries
            .iter_mut()
    ...
    pub(in super::super) fn remove(&mut self, handle: SlotHandle<Tag>) -> Option<T> {
        let entry = self.entries.get_mut(handle.index)?;
...
pub(in super::super) struct KeyedSlotStore<K, V, Tag> {
...
impl<K: Ord + Clone, V, Tag> KeyedSlotStore<K, V, Tag> {
    ...
    pub(in super::super) fn remove(&mut self, key: &K) -> Option<V> {
    ...
    pub(in super::super) fn keys(&self) -> impl Iterator<Item = &K> {
        self.by_key.keys()
    }

    /// translate a public key to the handle used by packet-loop queues
    pub(in super::super) fn handle_for_key(&self, key: &K) -> Option<SlotHandle<Tag>> {
    ...
    pub(in super::super) fn get_key_value_mut_by_handle(
        &mut self,
        handle: SlotHandle<Tag>,
    ) -> Option<(&K, &mut V)> {
        self.slots
