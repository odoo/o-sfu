...
fuzz_target!(|input: HttpRouteInput| {
    let token = SecretString::from(input.token.clone());
    let _ = verify::<HttpRoomClaims>(&token, &SecretString::from(TEST_AUTH_KEY));
    let _ = verify::<HttpDisconnectClaims>(&token, &SecretString::from(TEST_AUTH_KEY));
